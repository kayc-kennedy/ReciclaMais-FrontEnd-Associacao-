class Arvore {
  constructor(_raiz = null, _quantNos=0, resultadoInserir = true ) {
      this.raiz = _raiz;
      this.quantNos = _quantNos;
      this.resultadoInserir = resultadoInserir;
  }
  ...
  compara(_raiz, _valor) {
      if (_valor.id < _raiz.valor.id)
          return -1; // Inserir a esquerda
      else if (_valor.id > _raiz.valor.id)
  ...
  // Método para resgatar um catador na árvore pelo seu ID
  buscar(_id, _raiz=this.raiz) {

      //Verificar se a raiz atual é nula
      if(!this.isEmpty(_raiz)) {
          // Verificar os valores
          if(_id === _raiz.valor.id) {
              return _raiz.valor;
          }
          else if(_id < _raiz.valor.id) {
              return this.buscar(_id, _raiz.filhoEsq);
          }   
          else if(_id > _raiz.valor.id) { 
              return this.buscar(_id, _raiz.filhoDir);
          }
      }
...
class No {
  constructor(_valor) {
      this.valor = _valor; // Dado a ser inserido na arvore
      this.filhoEsq = null;
      this.filhoDir = null;
      this.hAE = 0;
      this.hAD = 0;
